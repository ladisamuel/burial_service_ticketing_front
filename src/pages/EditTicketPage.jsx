...
export default function EditTicketPage() {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      const res = await api.patch(`/tickets/request/edit/${state.id}/`, values)
      if (res?.data?.reference) {
        ...
        navigate(`/ticket/${res?.data?.reference}`)
      }
    } catch (err) {
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 md:p-8">
          <Formik
            initialValues={{
              ...
              email: state?.email || '',
              phone: state?.phone || '',
              number_of_guests: state?.number_of_guests || 1,
              relationship_to_deceased: state?.relationship_to_deceased || '',
            }}
          ...
          >
            {({ values, errors, touched, handleChange, handleBlur, isSubmitting }) => (
              <Form className="space-y-5">
                ...
                />
                <Input
                  label="Relationship to Deceased (optional)"
